...
&& (currColor.red != insideColor.red || currColor.green !=
insideColor.green || currColor.blue != insideColor.blue));
}
// check whether current pixel color is same as boundary color
int colorEqual(Color currentColor){
return (currentColor.green == boundColor.green && currentColor.blue ==
boundColor.blue && currentColor.red == boundColor.red);
}
...
if(colorEqual(getColorPixel(currX+1, currY)) || colorEqual(getColorPixel(currX,
currY+1))
|| colorEqual(getColorPixel(currX-1, currY)) ||
colorEqual(getColorPixel(currX, currY-1)))
continue;
...
// specifies values used by glClear to clear the buffer
